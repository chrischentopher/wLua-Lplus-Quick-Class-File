...
do
    ...
    ---@return void
    def.method("string", "string", 'string').samplePanel = function(self, name, path, functions)
        local className = 'UIPanel'..name
        ...
        strs = {
            "--返回resource_path.lua中的路径:",
            ("return RESPATH.%s"):format(path),
        }
        class:override("GetResPath", "", "string", "", make_function_body(strs), "", "")
        strs = {
            "--界面创建，事件监听，设置ui元件:",
    ...
    if false then
        ...
        --例子
        LplusFactory:samplePanel('TestCt1', "TestCt1", "Instance,onToggleObj,OnDestroy")
